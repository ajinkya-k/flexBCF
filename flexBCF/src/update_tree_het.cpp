...
void grow_tree_mu_het(tree &t, suff_stat &ss_train, int &accept, data_info &di_train, tree_prior_info &tree_pi, RNG &gen, bool prior_only, int min_node_size)
{
  ...
  rule_t rule;
  draw_rule(rule, t, nx_nid, di_train, tree_pi, gen, di_train.p_cont_mu, di_train.p_cat_mu);
  

  // at this point we have the proposed rule and are ready to update our sufficient statistic map
  suff_stat prop_ss_train;
  compute_suff_stat_grow_mu(ss_train, prop_ss_train, nx_nid, rule, t, di_train); // figure out which training observations from nx move to nxl and nxr
  

  int nxl_nid = 2*nx_nid; // id for the left child of nx
  int nxr_nid = 2*nx_nid+1; // id for right child of nx
  
  double nxl_lil = compute_lil_mu_het(prop_ss_train, nxl_nid, di_train, tree_pi); // nxl's contribution to log marginal likelihood of new tree
  double nxr_lil = compute_lil_mu_het(prop_ss_train, nxr_nid, di_train, tree_pi); // nxr's contribution to log marginal likelihood of new tree
  double nx_lil  = compute_lil_mu_het(ss_train, nx_nid, di_train, tree_pi); // nx's contribution to log marginal likelihood of old tree
  ...
  int l_count = prop_ss_train.find(nxl_nid)->second.size();
  int r_count = prop_ss_train.find(nxr_nid)->second.size();

  if(gen.log_uniform() <= log_alpha && l_count >= min_node_size && r_count >= min_node_size){
    ...
    if(rule.is_aa && !rule.is_cat){
      ...
      int v_raw = rule.v_cat + di_train.p_cont_mu;
...
void grow_tree_tau_het(tree &t, suff_stat &ss_train, int &accept, data_info &di_train, tree_prior_info &tree_pi, RNG &gen, bool prior_only, int min_node_size)
{
  std::vector<int> bn_nid_vec; // vector to hold the id's of all of the bottom nodes in the tree
  for(suff_stat_it ss_it = ss_train.begin(); ss_it != ss_train.end(); ++ss_it) bn_nid_vec.push_back(ss_it->first);
  ...
  // we now are ready to draw a decision rule
  rule_t rule;
  draw_rule(rule, t, nx_nid, di_train, tree_pi, gen, di_train.p_cont_tau, di_train.p_cat_tau);
  

  // at this point we have the proposed rule and are ready to update our sufficient statistic map
  suff_stat prop_ss_train;
  compute_suff_stat_grow_tau(ss_train, prop_ss_train, nx_nid, rule, t, di_train); // figure out which training observations from nx move to nxl and nxr
  
  int nxl_nid = 2*nx_nid; // id for the left child of nx
  int nxr_nid = 2*nx_nid+1; // id for right child of nx
  
  double nxl_lil = compute_lil_tau_het(prop_ss_train, nxl_nid, di_train, tree_pi); // nxl's contribution to log marginal likelihood of new tree
  double nxr_lil = compute_lil_tau_het(prop_ss_train, nxr_nid, di_train, tree_pi); // nxr's contribution to log marginal likelihood of new tree
  double nx_lil = compute_lil_tau_het(ss_train, nx_nid, di_train, tree_pi); // nx's contribution to log marginal likelihood of old tree
  ...
  if(gen.log_uniform() <= log_alpha && l_count >= min_node_size && r_count >= min_node_size){
    ...
    if(rule.is_aa && !rule.is_cat){
      ...
      int v_raw = rule.v_cat + di_train.p_cont_tau;
      ++(tree_pi.var_count->at(v_raw));
    } else if(!rule.is_aa && !rule.is_cat){
      ++(*tree_pi.rc_rule_count);
      for(rc_it it = rule.rc_weight.begin(); it != rule.rc_weight.end(); ++it){
...
void prune_tree_mu_het(tree &t, suff_stat &ss_train, int &accept, data_info &di_train, tree_prior_info &tree_pi, RNG &gen, bool prior_only)
{
  ...
  double nxl_lil = compute_lil_mu_het(ss_train, nxl_nid, di_train, tree_pi);
  double nxr_lil = compute_lil_mu_het(ss_train, nxr_nid, di_train, tree_pi);
  double nx_lil = compute_lil_mu_het(prop_ss_train, nx_nid, di_train, tree_pi);
  ...
  if(gen.log_uniform() <= log_alpha){
    ...
    if( nx->get_is_aa() && !nx->get_is_cat()){
     ...
     --(tree_pi.var_count->at(nx->get_v_aa()));
      ...
      int v_raw = di_train.p_cont_mu + nx->get_v_cat();
      --(tree_pi.var_count->at(v_raw));
    } else if( !nx->get_is_aa() && !nx->get_is_cat()){
...
void prune_tree_tau_het(tree &t, suff_stat &ss_train, int &accept, data_info &di_train, tree_prior_info &tree_pi, RNG &gen, bool prior_only)
{
  ...
  double nxl_lil = compute_lil_tau_het(ss_train, nxl_nid, di_train, tree_pi);
  double nxr_lil = compute_lil_tau_het(ss_train, nxr_nid, di_train, tree_pi);
  double nx_lil = compute_lil_tau_het(prop_ss_train, nx_nid, di_train, tree_pi);
  ...
  if(gen.log_uniform() <= log_alpha){
    ...
    if( nx->get_is_aa() && !nx->get_is_cat()){
     ...
     --(tree_pi.var_count->at(nx->get_v_aa()));
      ...
      int v_raw = di_train.p_cont_tau + nx->get_v_cat();
     --(tree_pi.var_count->at(v_raw));
    } else if( !nx->get_is_aa() && !nx->get_is_cat()){
      // random combination rule
      std::map<int,double> rc_weight = nx->get_rc_weight();
...
void update_tree_mu_het(tree &t, suff_stat &ss_train, int &accept, data_info &di_train, tree_prior_info &tree_pi, RNG &gen, bool prior_only, int min_node_size){
  ...
  if(gen.uniform() < PBx) grow_tree_mu_het(t, ss_train, accept, di_train, tree_pi, gen, prior_only, min_node_size);
...
void update_tree_tau_het(tree &t, suff_stat &ss_train, int &accept, data_info &di_train, tree_prior_info &tree_pi, RNG &gen, bool prior_only, int min_node_size)
{
  accept = 0; // initialize indicator of MH acceptance to 0 (reject)
  double PBx = tree_pi.prob_b; // prob of proposing a birth move (typically 0.5)
  if(t.get_treesize() == 1) PBx = 1.0; // if tree is just the root, we must always GROW
  
  if(gen.uniform() < PBx) grow_tree_tau_het(t, ss_train, accept, di_train, tree_pi, gen, prior_only, min_node_size);

...
double calculate_lp_diff(sigma_info &si, data_info &di, double log_prior_current, double log_prior_proposed, bool prior_only) {
  ...
  //NB: if prior_only is true, these sums are still their initial (0) values
...
void update_sigma_e(sigma_info &si, data_info &di, double nu, double lambda, double* wts, RNG& gen, bool prior_only) {
  ...
  double lp_diff = calculate_lp_diff(si, di, log_prior_current, log_prior_proposed, prior_only);
...
void update_mu_scale(sigma_info& si, data_info& di,
                      ...
                      double* mu_train, RNG& gen, bool prior_only) {
    ...
    double lp_diff = calculate_lp_diff_forscales(allfit, allfit_proposed, log_prior_current, log_prior_proposed, di, prior_only);
...
void update_tau_scale(sigma_info& si, data_info& di,
                      double* allfit, double* allfit_proposed, 
                      double* tau_train, RNG& gen, bool prior_only) {
